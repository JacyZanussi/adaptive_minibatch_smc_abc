...
def fvc_init(args,p=[2,None,0.5]):
    ...
    def update_mbs():
        ...
        print(f"Variance estimate: {2*est.v_total/N + (est.c*est.alpha_threshold)**2:.3f}. C estimate: {est.c_est:.3f}. V_total: {est.v_total:.3f}. batch size: {est.batch_size:.3f}. Time per sim: {est.step_time / est.num_sims:.3f}")
