...
def lotka_volterra_step(
    seed=None, sim_only = False,
    #a = 4, b = 0.01, g = 4, d = 1, #Physical parameters
    physical_params = [5,0.02,5], d = 1,
    input_filename = None, heterogeneities = [1000], #Heterogeneities
    scheme = 'constant', # Scheme
    scheme_params = [2], # Scheme hyperparameter
    stop_func = None,
    sample_size = 128, T = 5, dt = 0.01, #simulation parameters (base)
    alpha = 0.5, num_particles = 1000, cores = 4, #estimator parameters
    prior_domain = [[0.0,50.0],[0.0,0.2],[0.0,50.0]],
    **kwargs
):
    ...
    if input_filename is None:
        ...
        assert (ic_range >= 1) and (ic_range < 1001)
        shift = 500 - int((ic_range - 1)/2)
        ...
        if sim_only:
            save(f'datasets/lv_seed_{str(seed)}.pkl',data_temp)
            print(data_temp)
            return
    else:
...
def transcriptional_dynamics_step(
    ...
    physical_params = [15,10,0.1], kminus = 5,
    input_filename = None, #Heterogeneities - resimulate for 
    heterogeneities = [(2,2),(10.0,0.1)],
    scheme = 'constant', # Scheme
    scheme_params = [2], # Scheme hyperparameter
    stop_func = None,
    ...
    **kwargs
    ...
    if input_filename is None:
        ...
        if gp == None:
            ...
            gp_shape, gp_scale = gp
            lengths = rng.gamma(shape = gp_shape, scale = gp_scale, size = (sample_size,))
        if bp == None:
            sites = 0.5 * lengths
        else:
            bp_a,bp_b = bp
            sites = rng.beta(a = bp_a,b = bp_b, size = (sample_size,)) * lengths
        ## Simulate data
        ...
        if sim_only:
            save(f'datasets/td_seed_{str(seed)}.pkl',data)
            print(data)
            return
    else:
        print(f"Loading data from: {input_filename}")
...
def simulate_experiment(output_filename, parameter_set_list, model_name = None, num_sims=10,seeded = True):
    ...
    ## import currently available information to see if you can resume at a key.
    if os.path.exists(output_filename):
        print(f"Resuming from existing file: {output_filename}")
        sweep_dict = load(output_filename)
    else:
        sweep_dict = {}
    ...
    for p in parameter_set_list:
        p_key = to_key(p)
        if p_key in sweep_dict:
            print(f"Skipping {p_key}: Results already exist.")
            continue
        ...
        safe_save(output_filename,sweep_dict)
        print(f"Done with {p_key}. Progress saved.")
...
attr_list = [
    ...
    'hdpr_marginal', 'ESS',
    'posterior','posterior_stats','posterior_dists', 'weights', 'generation'
]
...
def safe_save(filename,data):
    ...
    try:
        save(temp_filename,data)
        os.replace(temp_filename, filename)
    except Exception as e:
        if os.path.exists(temp_filename):
            print(f"Save Failed. Temp file preserved at {temp_filename}")
        raise e
